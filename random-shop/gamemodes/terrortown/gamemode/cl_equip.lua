...
local function not_blocked (blocked_items, val)
    for index, value in ipairs(blocked_items) do
        if value == val then
            return false
        end
    end
...
hook.Add("TTTPrepareRound", "ResetRandomShopItems", resetRandomShopItems)
...
function GetEquipmentForRole(role)
   ...
   if not Equipment then
      ...
      for k, v in pairs(weapons.GetList()) do
         if v and v.CanBuy and not_blocked(blocked_items, v.PrintName) then
...
local function TraitorMenuPopup()
   ...
   if not next(randomShopItems) then
      i = 0
      repeat
		Yolo = tmpItems[math.random(4, table.maxn(tmpItems))]
         if not table.HasValue(items,Yolo) then
            table.insert(items,i,Yolo)
            i = i + 1
         end
      until (table.Count(items) >= (TTT_ITEMS + 1) - 1)
      randomShopItems = items
  else
      items = randomShopItems
  end

  items[0] = tmpItems[1] -- Armor
  items[1] = tmpItems[2] -- Radar
  items[2] = tmpItems[3] -- Disguiser

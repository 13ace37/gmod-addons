...
CreateConVar("ttt_firegrenade_no_fire", "1")
CreateConVar("ttt_firegrenade_no_dmg", "1")
...
function ENT:Explode(tr)
   if SERVER then
	  ...
	  if not GetConVar("ttt_firegrenade_no_dmg"):GetBool() then
	    util.Effect("Explosion", effect, true, true)
		util.BlastDamage(self, self:GetThrower(), pos, self:GetRadius(), self:GetDmg())
	  end
	  
	  if not GetConVar("ttt_firegrenade_no_fire"):GetBool() then
